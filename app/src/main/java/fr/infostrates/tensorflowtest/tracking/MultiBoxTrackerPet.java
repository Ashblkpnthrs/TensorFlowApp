...
package fr.infostrates.tensorflowtest.tracking;
...
import fr.infostrates.tensorflowtest.ClassifierPet;
import fr.infostrates.tensorflowtest.env.BorderedText;
import fr.infostrates.tensorflowtest.env.ImageUtils;
import fr.infostrates.tensorflowtest.env.Logger;
 ...
 */
public class MultiBoxTrackerPet {
    ...
    public MultiBoxTrackerPet(final Context context) {
    ...
    public synchronized void drawDebug(final Canvas canvas) {
        ...
        if (objectTrackerPet == null) {
            return;
        }

        // Draw correlations.
        for (final TrackedRecognition recognition : trackedObjects) {
            final ObjectTrackerPet.TrackedObject trackedObject = recognition.trackedObject;
        ...
        objectTrackerPet.drawDebug(canvas, matrix);
    }

    public synchronized void trackResults(
            final List<ClassifierPet.Recognition> results, final byte[] frame, final long timestamp) {
    ...
    public synchronized void onFrame(
        ...
        if (objectTrackerPet == null && !initialized) {
            ObjectTrackerPet.clearInstance();

            logger.i("Initializing ObjectTracker: %dx%d", w, h);
            objectTrackerPet = ObjectTrackerPet.getInstance(w, h, rowStride, true);
            frameWidth = w;
            frameHeight = h;
        ...
        objectTrackerPet.nextFrame(frame, null, timestamp, null, true);
    ...
    private void processResults(
            final long timestamp, final List<ClassifierPet.Recognition> results, final byte[] originalFrame) {
        final List<Pair<Float, ClassifierPet.Recognition>> rectsToTrack = new LinkedList<Pair<Float, ClassifierPet.Recognition>>();

        screenRects.clear();
        final Matrix rgbFrameToScreen = new Matrix(getFrameToCanvasMatrix());

        for (final ClassifierPet.Recognition result : results) {
        ...
        if (objectTrackerPet == null) {
            ...
            for (final Pair<Float, ClassifierPet.Recognition> potential : rectsToTrack) {
    ...
    private void handleDetection(
            final byte[] frameCopy, final long timestamp, final Pair<Float, ClassifierPet.Recognition> potential) {
        final ObjectTrackerPet.TrackedObject potentialObject =
                objectTrackerPet.trackObject(potential.second.getLocation(), timestamp, frameCopy);
